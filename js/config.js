...
export const CONFIG = {
  ...
  //   deep scan "Analyse night"   27 pts  ~133/hour             opt-in, 26 queries
  //   vet a character             17 pts  ~210/hour
  //   officer report card          5 pts  ~718/hour
  //   autopsy: open a report       1 pt  ~3529/hour
  //
  // The deep scan went 20 -> 24 -> 27 pts on 2026-08-07 (re-measured each time,
  // never estimated) as the Utility column grew: first nets/innervates/bombs,
  // then threat drops (more filtered Casts ids) and dispels (one more unfiltered
  // table). It stays the second-cheapest thing per use and is still opt-in.
  //
