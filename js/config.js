...
export const CONFIG = {
  ...
  //   deep scan "Analyse night"   35 pts  ~103/hour             opt-in, 34 queries
  //   vet a character             17 pts  ~210/hour
  //   officer report card          5 pts  ~718/hour
  //   autopsy: open a report       1 pt  ~3529/hour
  //
  // The deep scan went 20 -> 24 -> 27 -> 35 pts on 2026-09-03 (re-measured each
  // time, never estimated): first nets/innervates/bombs, then threat drops and
  // dispels, then the Tier 6 catalogue. That last step is the big one — adding
  // Hyjal/BT took the blameable spell ids from 33 to 73, which is 10 more
  // filtered DamageTaken batches.
  //
  // Worth knowing if this needs trimming: the batches are the SAME for every
  // report, so a Karazhan night still pays for every Black Temple id. Filtering
  // the id list by the report's zone would cut it back to roughly where it was.
  // Not done yet — it needs a zone tag per rule, which RULES doesn't carry.
  //
